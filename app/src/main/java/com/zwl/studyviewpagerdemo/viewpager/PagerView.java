...
public class PagerView extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                mScrollWidth += child.getMeasuredWidth();
        ...
        mScrollWidth = mScrollWidth - getMeasuredWidth();
        if (mScrollWidth < 0) {
            mScrollWidth = 0;
        }
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_UP:
                ...
                mLastMotionX = mInitialMotionX = ev.getX(ev.findPointerIndex(mActivePointerId));
                break;
        }
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                if (mIsBeingDragged) {
                    ...
                    final float pageOffset = 1.0f * (scrollX % width) / width;
                    final int activePointerIndex = ev.findPointerIndex(mActivePointerId);
                    final float x = ev.getX(activePointerIndex);
                    final int totalDelta = (int) (x - mInitialMotionX);
                    int nextPage = determineTargetPage(currentPage, pageOffset, initialVelocity,
                            totalDelta);
                    if (mOnScrollInterceptListener != null) {
                        if (mOnScrollInterceptListener.interceptScroll(currentPage + pageOffset, -totalDelta)) {
    ...
    private float interceptScroll(float deltaX) {
        if (deltaX != 0 && mOnScrollInterceptListener != null) {
            final int width = getClientWidth();
            final float scrollX = getScrollX() + deltaX;
            final float nextPage = scrollX / width;
            if (mOnScrollInterceptListener.interceptScroll(nextPage, deltaX > 0 ? 1 : -1)) {
                float roundPage = Math.round(nextPage);
                if (nextPage > roundPage) {
                    return roundPage * width - getScrollX();
                } else if (nextPage < roundPage) {
                    return roundPage * width - getScrollX();
                } else {
                    return 0;
                }
    ...
    private int determineTargetPage(int currentPage, float pageOffset, int velocity, int deltaX) {
        ...
        if (Math.abs(deltaX) > mFlingDistance && Math.abs(velocity) > mMinimumVelocity) {
            ...
            targetPage = pageOffset < 0.5f ? currentPage : currentPage + 1;
        }
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        if (velocity > 0) {
            ...
            final float pageWidth = width * 1;
            final float pageDelta = (float) Math.abs(dx) / (pageWidth + 1);
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode == View.OVER_SCROLL_ALWAYS
            ...
            if (!mRightEdge.isFinished()) {
                ...
                canvas.rotate(90);
                canvas.translate(-getPaddingTop(), -1 * getChildCount() * width);
...
//                        pageScrolled(x);
    ...
    public void setOnScrollInterceptListener(OnScrollInterceptListener listener) {
        this.mOnScrollInterceptListener = listener;
    }
    ...
    public interface OnScrollInterceptListener {
         ...
         * 滑动拦截
         *
         * @param pageOffset 当前 position
         * @param deltaX     滑动方向,1为正向滑动, -1为反向滑动
         * @return true 为拦截(不可滑动), false 为不拦截(可以滑动)
         */
        boolean interceptScroll(float pageOffset, int deltaX);
    }
}
